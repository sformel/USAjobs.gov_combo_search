...
ui <-  fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      textInput(
        "skipwords",
        "Exclude jobs with this word in the title (separate with commas):",
        ""
      ),
  ...
  observeEvent(input$search, 
               {
                 ...
                 KW_list <- strsplit(input$keywords, ",|, ") %>% 
                   unlist()
                 skip_list <- strsplit(input$skipwords, ",|, ") %>% 
                   unlist()
                 URL_list <- paste0("https://data.usajobs.gov/api/search?Keyword=",
                                    KW_list,
                                    "&ResultsPerPage=1000")
                 #Looped requests
                 ...
                 for (i in 1:length(df.list)) {
                   results.list[[i]] <- df.list[[i]]$SearchResult$SearchResultItems$MatchedObjectDescriptor %>%
                     filter(!(str_detect(tolower(PositionTitle),
                                         paste0(skip_list, collapse = "|")
                                         )),
                            PositionStartDate > Sys.Date() - post_days) %>% #date minus N days
                     select(
                       PositionID,
                       OrganizationName,
                       ApplyURI,
                 ...
                 results.list <- results.list[lapply(results.list, 
                                                     nrow) %>%
                                                unlist() > 0]
                 
                 for (i in 1:length(results.list)) {
                   results.html.list[[i]] <- data.frame(
                     "Org" = results.list[[i]]$OrganizationName,
